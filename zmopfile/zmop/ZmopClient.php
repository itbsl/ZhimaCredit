...
include('./zmopfile/zmop/RSAUtil.php');
include('./zmopfile/zmop/WebUtil.php');
include('./zmopfile/lotusphp_runtime/Logger/Logger.php');
class ZmopClient {
	//应用ID
	public $appId;
	//私钥文件路径，绝对路径或者相对路径
	...
	public function execute($request) {
		...
		if (isset ($bizRespObject->error_code)) {
			$logger = new LtLogger;
			$logger->conf["log_file"] = './zmopfile/'.rtrim(ZMOP_SDK_WORK_DIR, '\\/') . '/' . "logs/zmop_biz_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
					date("Y-m-d H:i:s"),
					$resp
				));
	...
	private function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		$logger = new LtLogger;
		$logger->conf["log_file"] = './zmopfile/'.rtrim(ZMOP_SDK_WORK_DIR, '\\/') . '/' . "logs/zmop_comm_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
